...
class Grid:
    ...
    def is_dead_end(self, x: int, y: int) -> bool:
        ...
        road_neighbors = sum(
            1 for nx, ny in self.get_neighbors(x, y) if self.is_road_cell(nx, ny)
        )
                ...
                while queue:
                    ...
                    for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                        ...
                        if self.is_inside_grid(nx, ny) and (nx, ny) not in visited:
                            if (
                                self.is_building_cell(nx, ny)
                                and int(self.matrix[nx][ny]) == building_id
                            ):
                                queue.append((nx, ny))
